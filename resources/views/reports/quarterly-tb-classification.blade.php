  ...
  <div class="header" id="header">
    ...
    <div class="main-content py-4" id="mainContent">
      ...
      <div class="d-flex justify-content-end mb-2 gap-1">
        <a href="{{ route('pdf.quarterly-tb-classification.pdf') }}" target="_blank" class="btn btn-danger">
            ...
            <option value="{{ url('quarterly-treatment-outcome') }}">Treatment Outcome</option>
              ...
              <thead>
                <tr>
                  <th>Year</th>
                  <th>Quarter</th>
                  <th>Bacteriologically Confirmed TB</th>
                  <th>Clinically Diagnosed TB</th>
                  <th>Total Cases</th>
                </tr>
              </thead>
              <tbody>

              @foreach ($quarterlyTBClassification as $patient)
                
                <tr>
                  <td>{{ $patient->year }}</td>
                  <td>{{ $patient->quarter }}</td>
                  <td>{{ $patient->bacteriological }}</td>
              ...
              <div class="card-footer">Showing {{ $quarterlyTBClassification->firstItem() }} to {{ $quarterlyTBClassification->lastItem() }} of
        {{ $quarterlyTBClassification->total() }} entries
        <div class="mt-2">
          {{ $quarterlyTBClassification->links() }}
        </div>
      </div>
